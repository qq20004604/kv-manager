...
class KeyValueManager(object):
    ...
    def __init__(self, redis_args, mysql_args):
        ...
        self.kv_pool = {}
        # 当从mysql读取时，写入redis里时，redis的key的过期时间（单位秒）
        self.REDIS_EXPIRE_TIME = 600

        t1 = threading.Thread(target=self._eventloop, name='loop')
        t1.start()
        # print('__init__ end')

    def set_redis_expire_time(self, seconds):
        self.REDIS_EXPIRE_TIME = seconds

    # 注册监听一个key
    # key: key值，必须是string类型
    # default_val: 默认值，必须是string类型，必须设置
    # expire_time：过期时间，默认值为10（单位s），这里指本地过期时间
    def register_key(self, key, default_val, expire_time=10):
    ...
    def update_keys(self, keys):
        ...
        for item in list:
            ...
            if item['is_error'] is not True:
                ...
                # 错误的则设置重置该key的过期时间（值不变），在下次查询的时候再次查询，并写入日志（只写一次），但更新不止一次
        ...
        try:
            ...
            for item in list:
                if item['is_error'] is False:
                    k = item['key']
                    r.set(k, item['value'], ex=self.REDIS_EXPIRE_TIME)
        except BaseException as e:
            err = str(e)
            log(str(err))
...
if __name__ == '__main__':
    ...
    # 注册key
    c.register_key('foo', 'default foo', expire_time=10)
